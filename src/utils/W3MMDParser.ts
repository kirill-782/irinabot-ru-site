...
export class W3MMDStatsParser {
    ...
    private processAction(action: SyncIntegerData, fromPid: number, time?: number) {
                    ...
                    for (let i = 0; i < tokens.length - 2; ++i) {
                        if (eventDef[i].startsWith("pid:")) {
                            ...
                            if (this.state?.playerToName?.[playerId] === undefined) {
                                ...
                                format = format.replace(`{${i}}`, this.state?.playerToName?.[playerId]);
                            }
                        } else {
    ...
    private tokenizeKey(key: string) {
        ...
        for (let i = 0; i < key.length; ++i) {
            if (escaping) {
                ...
                if (key[i] === " ") {
                    if (token.length === 0) throw new Error("Error tokenizing key [" + key + "], empty token found.");
