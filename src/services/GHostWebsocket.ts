...
import { DEFAULT_CONTEXT_HEADER_CONSTANT, DEFAULT_GAME_LIST, DEFAULT_MAP_INFO, DEFAULT_WEBSOCKET_CONNECT_STATS } from '../models/websocket/HeaderConstants';
...
const packageHandlers = (() => {
    ...
    handlers[DEFAULT_CONTEXT_HEADER_CONSTANT][DEFAULT_WEBSOCKET_CONNECT_STATS] = new ServerWebsocketConnectStatsConverter();
...
export class GHostWebSocket extends EventTarget {
    ...
    {
        setTimeout(() => {
            if(this.socketConnect.readyState == WebSocket.CLOSED)
                this.reconnect( );
