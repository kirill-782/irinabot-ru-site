...
import { DEFAULT_AUTOHOST_LIST_RESPONSE, DEFAULT_CONTEXT_HEADER_CONSTANT, DEFAULT_GAME_LIST } from "./HeaderConstants";
...
export class ServerAutohostListResponseConverter extends AbstractConverter {
    public assembly(data: ServerAutohostListResponse) {
        ...
        data.autohosts.forEach(
            (element) => dataBuffer.putByteArray(new Uint8Array(autohostsConverter.assembly(element))),
            this
        );
    ...
    public parse(dataBuffer: DataBuffer): ServerAutohostListResponse {
        ...
        for (let i = 0; i < countAutohosts; ++i) autohosts[autohosts.length] = autohostsConverter.parse(dataBuffer);
