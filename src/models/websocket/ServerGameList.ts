...
import { DEFAULT_CONTEXT_HEADER_CONSTANT, DEFAULT_GAME_LIST } from "./HeaderConstants";
...
export class ServerGameListConverter extends AbstractConverter {
    public assembly(data: ServerGameList) {
        ...
        data.games.forEach(
            (element) => dataBuffer.putByteArray(new Uint8Array(gameListGameConverter.assembly(element))),
            this
        );
    ...
    public parse(dataBuffer: DataBuffer): ServerGameList {
        ...
        for (let i = 0; i < countGames; ++i) games[games.length] = gameListGameConverter.parse(dataBuffer);
...
class GameListGameConverter {
    public assembly(data: GameListGame) {
        ...
        dataBuffer.putNullTerminatedString(data.gameVersion);
        ...
        data.players.forEach(
            (element) => dataBuffer.putByteArray(new Uint8Array(gameListPlayerConverter.assembly(element))),
            this
        );
    ...
    public parse(dataBuffer: DataBuffer): GameListGame {
        ...
        const gameVersion = dataBuffer.getNullTerminatedString();
        ...
        for (let i = 0; i < countPlayers; ++i) players[players.length] = gameListPlayerConverter.parse(dataBuffer);
