...
export const useConnectorIdCache = ({
    ghostSocket,
}: ConnectorIdProps): [CachedConnetcorIds, React.Dispatch<CacheConnectorIdAction>] => {
    const authReducer = (state: CachedConnetcorIds, action: CacheConnectorIdAction) => {
    ...
    const [cachedConnectorIds, dispatchCacheConnectorId] = useReducer(authReducer, { "0": "Global Scope" });
