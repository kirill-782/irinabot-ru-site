  ...
  useEffect(() => {
    const onUDPGameAddPackage = (data) => {
      if (
        ...
        gameParams.append(
          "mapGameFlags",
          fromByteArray(Uint8Array.from(gameData.mapGameFlags))
        );
        gameParams.append(
          "mapWidth",
          fromByteArray(Uint8Array.from(gameData.mapWidth))
        );
        gameParams.append(
          "mapHeight",
          fromByteArray(Uint8Array.from(gameData.mapHeight))
        );
        gameParams.append(
          "mapCrc",
          fromByteArray(Uint8Array.from(gameData.mapCrc))
        );
        gameParams.append("mapPath", gameData.mapPath);
        gameParams.append("hostName", gameData.hostName);
        gameParams.append(
          "mapFileSha1",
          fromByteArray(Uint8Array.from(gameData.mapFileSha1))
        );
        ...
        if (gameData.broadcast) {
          gameParams.append(
            "productId",
            fromByteArray(Uint8Array.from(gameData.productId))
          );
          gameParams.append(
            "versionPrefix",
            fromByteArray(Uint8Array.from(gameData.versionPrefix))
          );
