...
export const useSearchFiltersQuerySync = (
    ...
    useEffect(() => {
        if (search !== oldSearch) {
            ...
            updateParam(queryParser, "taggedOnly", filters[0]?.taggedOnly?.toString());
            updateParam(queryParser, "favorite", filters[0]?.favorite?.toString());
            updateParam(queryParser, "minPlayers", filters[0]?.minPlayers?.toString());
            updateParam(queryParser, "maxPlayers", filters[0]?.maxPlayers?.toString());
...
const updateParam = (queryParser: URLSearchParams, key: string, value?: string) => {
