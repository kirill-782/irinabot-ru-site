...
export const useGameListFilterSetings = () => {
    ...
    const disableFilter = (filterName: string, value?: any) => {
        setDisabledFilters((disabledFilters) => {
            if (disabledFilters.indexOf(filterName) === -1) return [...disabledFilters, filterName];
    ...
    useEffect(() => {
        const storedFilterSettings = localStorage.getItem(gameListFilterLocalStorageKey);
    ...
    useEffect(() => {
        localStorage.setItem(gameListFilterLocalStorageKey, JSON.stringify(filterSettings));
    }, [filterSettings]);
