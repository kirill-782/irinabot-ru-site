...
function MapInfo({ mapId }: MapInfoProps) {
    ...
    return (
        <Container className="map-info">
            ...
            />
            <div>
                <span className="map-players-title">{lang.mapInfoPlayersCount}</span>
                <WarcraftIIIText>{mapInfo?.mapInfo?.playerRecommendation || ""}</WarcraftIIIText>
                <div>
                    <WarcraftIIIText>{mapInfo?.mapInfo?.description || ""}</WarcraftIIIText>
                </div>
            </div>
        </Container>
    );
