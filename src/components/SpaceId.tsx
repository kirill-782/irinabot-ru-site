...
function SpaceId({ requiredAccessMask, value, onChange, ...props }: SpaceIdProps & StrictFormFieldProps) {
    const { accessMask, currentAuth } = useContext(AuthContext).auth;

    const hasGlobalAccess = accessMask.getRecords().some((i) => {
        return i.spaceId === 0 && (i.accessMask & requiredAccessMask) === requiredAccessMask;
    });
    ...
    return (
        <Form.Select
            ...
            {...props}
            onChange={(_, data) => {
                if (onChange) onChange(parseInt(data.value?.toString() || currentAuth.connectorId.toString()));
            }}
        ></Form.Select>
    );
}
