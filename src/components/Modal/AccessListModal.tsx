...
function AccessListModal({ open, onClose }: AccessListModalProps) {
    ...
    const [showAccessMask, setShowAccessMask] = useState<number | undefined>(undefined);
            ...
            <Modal.Content>
                ...
                {accessRows.length === 0 ? (
                    ...
                    <>
                        ...
                        <Table>
                            <Table.Header>
                                <Table.HeaderCell>{lang.accessListModalPlayerId}</Table.HeaderCell>
                                <Table.HeaderCell>{lang.accessListModalAccessMask}</Table.HeaderCell>
                                <Table.HeaderCell>{lang.accessListModalExpire}</Table.HeaderCell>
                            </Table.Header>
                            {accessRows.map((i) => {
                                return (
                                    <Table.Row key={i.spaceId}>
                                        ...
                                        >
                                            <span className="access-list-modal access-mask">{i.accessMask}</span>
                                        </Table.Cell>
                                        <Table.Cell>
                                            {i.expireTime === 0
