        ...
        <Grid className="replay-main-info">
            <Grid.Row stretched>
                ...
                <Segment className="fluid">
                    ...
                    <div>
                        <b>Version:</b>
                        {replayData?.subHeader.version} build {replayData?.subHeader.buildNumber}
                    </div>
                    <div>
                        <LanguageKey
                            stringId="replayMainInfo_2"
                            value={prettyMilliseconds(replayData?.subHeader.lengthMilis || 0)}
                        />
                    </div>
                    <div>
                        <LanguageKey
                            stringId="replayMainInfo_2"
                            value={replayData?.records.gameInfo?.hostPlayer.playerName}
                        />
                    </div>
                </Segment>
                <Header>
                    <LanguageKey
                        stringId="replayMainInfo_6"
                        value={replayData?.records.gameInfo?.hostPlayer.playerName}
                    />
                </Header>
                <Table>
                    <TableHeader>
                        <TableHeaderCell>{t("replayMainInfo_7")}</TableHeaderCell>
                        <TableHeaderCell>{t("replayMainInfo_8")}</TableHeaderCell>
                        <TableHeaderCell>{t("replayMainInfo_9")}</TableHeaderCell>
                    </TableHeader>
                    <TableBody>
                        {[...(replayData?.records.players || []), replayData?.records.gameInfo?.hostPlayer].map((i) => {
                            const leftRow = getLeaveRowByPID(i?.playerId);

                            return (
                                <TableRow key={i?.playerId}>
                                    <TableCell>{i?.playerName}</TableCell>
                                    <TableCell>{prettyMilliseconds(leftRow?.time || 0)}</TableCell>
