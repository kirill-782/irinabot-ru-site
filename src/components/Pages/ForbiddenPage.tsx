...
import { Button, Container, Dropdown, Grid, Header, Loader, Segment } from "semantic-ui-react";
...
function ForbiddenPage({ missingAuthorities, missingAuth }: ForbiddenPageProps) {
        ...
        <>
            ...
            <Grid.Row>
                {AviableAuthMethods.map((method: AuthMethod) => {
                    return (
                        <Button key={method.name} color={method.color} onClick={() => authByOauth(method, onSuccess)}>
                            {method.name}
                        </Button>
                    );
                })}
